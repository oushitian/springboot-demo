/**
 * jquery.easyui.validate.extend.js
 * 
 */
			...
			for (var i = 0; i < vtypes.length; i++) {
				...
				if (value && rule) {
					...
					if (!rule["validator"](value, parame)) {
						returnFlag = false;
//						opts.rules.multiple.message = rule.message;
	...
	mobile : {// 验证手机号码
		validator : function(value) {
			return /^((\+)?\d*)(13|15|17|18)\d{9}$/i.test(value);
		},
		message : '手机号码格式不正确(正确格式如：18888888888或8618888888888或+8618888888888)'
	},
	phoneOrMobile : {// 验证手机或电话
		validator : function(value) {
			return /^((\+)?\d*)(13|15|17|18)\d{9}$/i.test(value) || /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(value);
		},
	...
	remote : {
		validator : function(value, param) {
			var windowStatus = $('#windowStatus').val();
			if (windowStatus && windowStatus != 'window_status_new' && windowStatus != 'window_status_edit') {
				return true;
			}
			var data = {};
			data[param[1]] = value;
